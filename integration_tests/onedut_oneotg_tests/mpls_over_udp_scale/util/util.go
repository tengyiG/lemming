...
func populateNextHops(cfg *ScaleProfileConfig, niName string, niIndex int, numNHsThisNI int, nhIndexOffset uint64) ([]fluent.GRIBIEntry, error) {
...
func populateNextHopGroups(cfg *ScaleProfileConfig, niName string, numNHGsThisNI int, numNHsPerGroup int, nhgIDOffset uint64, nhIndexOffset uint64) ([]fluent.GRIBIEntry, error) {
	numNHsThisNI := cfg.NumNexthopPerNHG * cfg.NumNexthopGroup
	k := cfg.NumNexthopPerNHG // NHs per group
	entries := []fluent.GRIBIEntry{}
...
func populatePrefixes(cfg *ScaleProfileConfig, niName string, numPrefixesThisNI int, numNHGsPerNI int, nhgIDOffset uint64) ([]fluent.GRIBIEntry, error) {
	entries := []fluent.GRIBIEntry{}
...
func GenerateScaleProfileEntries(ctx context.Context, cfg *ScaleProfileConfig) ([]fluent.GRIBIEntry, error) {
	...
	for niIndex := 0; niIndex < cfg.NumNetworkInstances; niIndex++ {
		...
		nhs, err := populateNextHops(cfg, niName, niIndex, numNHsPerNI, nhIndexOffset)
		if err != nil {
			return nil, fmt.Errorf("failed to populate next hops for NI %s (index %d): %w", niName, niIndex, err)
		}
		allEntries = append(allEntries, nhs...)

		nhgs, err := populateNextHopGroups(cfg, niName, numNHGsPerNI, numNHsPerGroup, nhgIDOffset, nhIndexOffset)
		if err != nil {
		...
		prefixEntries, err := populatePrefixes(cfg, niName, numPrefixesPerNI, numNHGsPerNI, nhgIDOffset)
		if err != nil {
