...
func configureDUT(t *testing.T, dut *ondatra.DUTDevice) {
	...
	niState := gnmi.Get(t, dut, ocpath.Root().NetworkInstance(TestNI).State())
	niStateStr, err := ygot.EmitJSON(niState, &ygot.EmitJSONConfig{
		Format: ygot.RFC7951,
		Indent: "  ",
	...
	if err != nil {
		t.Logf("Error marshalling NI state to JSON: %v", err)
		t.Logf("Raw NI state for %q: %+v", TestNI, niState)
	} else {
		t.Logf("Current state for NI %q:\n%s", TestNI, niStateStr)
	}
}
...
func validateAFTState(t *testing.T, dut *ondatra.DUTDevice, cfg *scaleutil.ScaleProfileConfig, expectPresent bool) {
	...
	for niIndex := 0; niIndex < cfg.NumNetworkInstances; niIndex++ {
		...
		for _, nhIndex := range nhIndicesToCheck {
			t.Logf("Checking NH index: %d in NI %s (expect %s)", nhIndex, niName, expectation)
			nhPath := ocpath.Root().NetworkInstance(niName).Afts().NextHop(nhIndex) // Use correct niName
			...
			if expectPresent {
				...
				// Construct expected Encap Headers based on config and niIndex
				expectedMPLSLabel := cfg.MPLSLabelStart
				if !cfg.UseSameMPLSLabel {
					expectedMPLSLabel = cfg.MPLSLabelStart + uint64(niIndex) // Label depends on VRF index
				}
		...
		for _, nhgIndex := range nhgIndicesToCheck {
			t.Logf("Checking NHG index: %d in NI %s (expect %s)", nhgIndex, niName, expectation)
		...
		for i, prefixStr := range prefixesToCheck {
			t.Logf("Checking Prefix: %s in NI %s (expect %s)", prefixStr, niName, expectation)
...
func TestMPLSOverUDPScale(t *testing.T) {
	...
	tests := []struct {
		...
		{
			desc: "Profile 2 (P1 General - Multi-VRF) - Basic Scale",
			config: &scaleutil.ScaleProfileConfig{
				AddrFamily:              "ipv6",
